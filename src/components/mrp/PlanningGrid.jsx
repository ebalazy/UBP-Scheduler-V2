...
export default function PlanningGrid({
    ...
    const renderMobileCard = (date) => {
        ...
        // 2. Actual
        const actualVal = monthlyProductionActuals[dateStr];
        const hasActual = actualVal !== undefined;

        // 3. Inbound
        const manifestItems = poManifest[dateStr]?.items || [];
        const hasManifest = manifestItems.length > 0;
        const inboundVal = hasManifest ? manifestItems.length : (monthlyInbound[dateStr] || 0);

        // 4. Balance & Coverage
        const ledgerItem = ledgerMap[dateStr];
        const balance = ledgerItem ? ledgerItem.balance : null;

        // Coverage Calc (Simplified for Mobile Display)
        let coverageLabel = '-';
        let coverageColor = 'text-gray-400';

        if (balance !== null) {
            // Re-use logic or just grab from ledger if pre-calculated? 
            // We calculated coverage in the render loop previously. 
            // Let's re-calc briefly or extract logic. For now, simple re-calc.

            // ... existing coverage logic partial ...
            // For speed/cleanliness in mobile, maybe just show Balance?
            // User asked for "Activity Ledger", implying inputs are key.
        }

        return (
            <div key={dateStr} className={`p-4 rounded-xl border-2 mb-4 shadow-sm ${isToday ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' : 'bg-white border-gray-100 dark:bg-gray-800 dark:border-gray-700'}`}>
                {/* Header */}
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                        ...
                        <input
                            ...
                            placeholder="-"
                            value={demandVal || ''}
                            onChange={e => updateDateDemand(dateStr, e.target.value)} // Simplified for mobile
                        />
                    </div>
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            {/* MOBILE LIST VIEW */}
            <div className="md:hidden flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-900 space-y-4">
                <div className="text-xs text-center text-gray-400 mb-2 uppercase font-bold tracking-widest">
                    Planning Horizon (Next 30 Days)
                </div>
                {dates.slice(2, 32).map(date => renderMobileCard(date))}
                <div className="text-center text-xs text-gray-400 py-4">
                    Switch to Desktop for full 45-day view & bulk tools.
                </div>
            </div>

            {/* DESKTOP TABLE VIEW */}
            <div className="hidden md:block flex-1 overflow-auto relative" ref={scrollRef}>
            ...
            <div className="hidden md:block p-2 bg-gray-50 text-[10px] text-gray-400 text-center dark:bg-gray-800">
